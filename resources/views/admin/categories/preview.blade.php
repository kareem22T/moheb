...
<div class="card w-100" id="lang_prev">
    ...
    <h4 class="text-center">
        @{{ laguages_data && languages_data.data.length == 0 ? 'There is no any added language yet' : '' }}
    </h4>
    <h4 class="text-center">
...
createApp({
    ...
    methods: {
        ...
        async getLanguages() {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/admin/languages`, {
                },
                );
                if (response.data.status === true) {
                    $('.loader').fadeOut()
        ...
        async getSearch(search_words) {
            try {
                const response = await axios.post(`/admin/languages/search`, {
